...
// handler.ts
var DataTarget = /* @__PURE__ */ ((DataTarget2) => {
  DataTarget2["COUNT"] = "count";
  DataTarget2["AVG_AUTONOMY"] = "avg_autonomy";
  return DataTarget2;
})(DataTarget || {});
var zRequestPayload = import_zod.z.object({
...
var zProcessorApiDatum = import_zod.z.union([import_zod.z.null(), import_zod.z.string(), import_zod.z.number()]).array();
var zProcessorApiData = import_zod.z.object({
  meta: import_zod.z.object({
    view: import_zod.z.object({
...
var Processor = class {
  constructor(payload) {
    this.API_URL = "https://data.wa.gov/api/views/f6w7-q2d2/rows.json";
    this.carmakerColumnName = "Make";
    this.electricRangeColumnName = "Electric Range";
    this.evTypeColumnName = "Electric Vehicle Type";
    this.dataTarget = import_zod.z.nativeEnum(DataTarget).parse(JSON.parse(payload.body));
  }
  async process() {
    switch (this.dataTarget) {
      case "count" /* COUNT */:
        return this.countCarsByCarMakers();
      case "avg_autonomy" /* AVG_AUTONOMY */:
        return this.avgAutonomyByCarMakers();
      default:
        throw new Error(`Unsupported data target ${this.dataTarget}`);
    }
  }
  async getDataAndColumns() {
    const res = await axios_default.get(this.API_URL);
    const parsedResponseData = zProcessorApiData.parse(res.data);
    return {
      columns: parsedResponseData.meta.view.columns.map((c) => c.name),
      data: parsedResponseData.data
    };
  }
  async countCarsByCarMakers() {
    const { columns, data } = await this.getDataAndColumns();
    const carMakerColumnIndex = columns.indexOf(this.carmakerColumnName);
    const countByCarMakers = {};
  ...
  async avgAutonomyByCarMakers() {
    const { columns, data } = await this.getDataAndColumns();
    const carMakerColumnIndex = columns.indexOf(this.carmakerColumnName);
    const avgAutonomyByCarMakers = {};
    const dataByCarMakers = this.dataCarsByCarMakers(carMakerColumnIndex, data);
    ...
    for (const k of Object.keys(dataByCarMakers)) {
      const electricCars = dataByCarMakers[k].filter((d) => d[evTypeColumnIndex] === "Battery Electric Vehicle (BEV)");
      if (electricCars.length === 0) {
        avgAutonomyByCarMakers[k] = 0;
        continue;
      }
      avgAutonomyByCarMakers[k] = electricCars.reduce((acc, d) => {
        const electricRange = d[electricRangeColumnIndex];
        if (typeof electricRange !== "string")
          throw new Error(`Electric range is not a string for datum ${d} at index ${electricRangeColumnIndex}: ${d[electricRangeColumnIndex]}`);
        return acc + parseInt(electricRange);
      }, 0) / electricCars.length;
    }
    return avgAutonomyByCarMakers;
  }
  dataCarsByCarMakers(carMakerColumnIndex, data) {
    const dataByCarMakers = {};
    for (const d of data) {
      const carmaker = d[carMakerColumnIndex];
      if (typeof carmaker !== "string")
        throw new Error(`Car maker is not a string for datum ${d} at index ${carMakerColumnIndex}: ${d[carMakerColumnIndex]}`);
      if (dataByCarMakers[carmaker] === void 0)
        dataByCarMakers[carmaker] = [];
